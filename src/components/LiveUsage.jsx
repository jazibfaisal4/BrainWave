    ...
    >
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2.5">
          ...
          <h4 className="text-base font-semibold text-n-1">{GPU_STATS.label}</h4>
        </div>
        <span className="text-sm font-code text-n-3">
          {GPU_STATS.used.toLocaleString()} / {GPU_STATS.total.toLocaleString()} {GPU_STATS.unit}
        </span>
      </div>
      ...
      <div className="relative h-4 w-full rounded-full bg-n-6/60 overflow-hidden">
        <motion.div
          ...
          style={{
            background: "linear-gradient(90deg, #AC6AFF 0%, #79FFF7 50%, #FFC876 100%)",
          }}
          initial={{ width: "0%" }}
          animate={isInView ? { width: `${GPU_STATS.percentage}%` } : { width: "0%" }}
        ...
        <motion.div
          ...
          transition={{ duration: 2, repeat: Infinity, repeatDelay: 1, ease: "linear", delay: 1.5 }}
        />
      </div>

      <div className="flex items-center justify-between mt-3">
        <span className="text-xs text-n-4 font-code">0</span>
        <span className="text-sm font-bold text-color-1">{GPU_STATS.percentage}% utilized</span>
        <span className="text-xs text-n-4 font-code">{GPU_STATS.total.toLocaleString()}</span>
      </div>
    </motion.div>
  );
};
...
const MetricCard = ({ metric, index }) => (
  <motion.div
    variants={fadeInUp}
    custom={index + 2}
    className="group rounded-2xl border border-n-6/40 bg-n-7/60 backdrop-blur-xl p-5
               transition-all duration-400 hover:border-color-1/30 hover:shadow-[0_0_24px_rgba(172,106,255,0.1)]"
  >
...
const ActivityFeed = () => {
  const [activities, setActivities] = useState(() =>
    ACTIVITY_MESSAGES.slice(0, 4).map((msg, i) => ({
      ...msg,
      id: `init-${i}`,
      timestamp: new Date(Date.now() - (4 - i) * 60_000),
    }))
  );
    ...
    >
      ...
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-2.5">
          ...
          <h4 className="text-base font-semibold text-n-1">Real-time Activity</h4>
        </div>
      ...
      <div className="space-y-3 flex-1 overflow-hidden">
        <AnimatePresence initial={false}>
          {activities.map((act) => (
            ...
            >
              <span
                className={`mt-1 h-2 w-2 rounded-full shrink-0 ${typeColors[act.type] || "bg-n-4"}`}
              />
              <div className="min-w-0 flex-1">
                <p className="text-sm text-n-2 leading-snug truncate">
                  <span className="font-semibold text-n-1">@{act.user}</span>{" "}
                  {act.action}
                </p>
  ...
  >
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
      {STATUS_ITEMS.map((item) => (
        <div
          key={item.id}
          className="flex items-center gap-2.5 rounded-xl bg-n-8/60 border border-n-6/20 px-3.5 py-3 transition-all duration-300 hover:border-n-5/40"
        >
          <span
            className={`h-2 w-2 rounded-full shrink-0 ${
              item.status === "operational" ? "bg-color-4" : "bg-color-2 animate-pulse"
            }`}
          />
          <div>
            ...
            <p
              className={`text-[0.6rem] uppercase tracking-wider font-bold ${
                item.status === "operational" ? "text-color-4" : "text-color-2"
              }`}
            >
              {item.status}
            </p>
    ...
    <Section id="live-usage" crosses>
      <div className="container relative z-2">
        ...
        >
          ...
          {/* Row 3: Activity feed (full width on md, 2 cols on lg) */}
          <ActivityFeed />
        </motion.div>
      </div>
    </Section>
